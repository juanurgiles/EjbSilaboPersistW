...
public abstract class AbstractFacade<T> {
    ...
    public List<T> buscarM(String campo, Integer valor) {
        TypedQuery<T> query = getEntityManager().createQuery(
                "SELECT c FROM " + entityClass.getName() + " c WHERE c." + campo + " = ?1", entityClass);


        return query.setParameter("1", valor).getResultList();
    }
}
